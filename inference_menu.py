...
if if_sep_reverb == 'y':
    ...
    if reverb_model_name == '5':
        reverb_model_name = 'dereverb_mel_band_roformer_anvuew_sdr_19.1729.ckpt'
    if reverb_model_name == '6':
        reverb_model_name = 'dereverb_mel_band_roformer_less_aggressive_anvuew_sdr_18.8050.ckpt'
...
if if_sep_kara == 'y':
    print('================开始分离和声================')
    inference_case = inference_base + ' --model_type mel_band_roformer' + f' --start_check_point pretrain/{kara_model_name}' + f' --input_folder {module_inputs["karaoke_input"]} --store_dir karaoke_results --extract_instrumental'
    if if_fast == 'y':
...
if if_sep_reverb == 'y':
    print('================开始分离混响和声================')
    inference_case = inference_base + f' --start_check_point pretrain/{reverb_model_name}' + f' --input_folder {module_inputs["reverb_input"]} --store_dir deverb_results --extract_instrumental'
    ...
    if reverb_model_name == 'dereverb_mel_band_roformer_anvuew_sdr_19.1729.ckpt':
        if if_fast == 'y':
    ...
    if reverb_model_name == 'dereverb_mel_band_roformer_less_aggressive_anvuew_sdr_18.8050.ckpt':
        if if_fast == 'y':
            inference_case = inference_case + ' --config_path configs/dereverb_mel_band_roformer_anvuew-fast.yaml  --model_type mel_band_roformer'
        else:
            inference_case = inference_case + ' --config_path configs/dereverb_mel_band_roformer_anvuew.yaml  --model_type mel_band_roformer'
...
if if_denoise == 'y':
    print('================开始降噪================')
    inference_case = inference_base + f' --model_type mel_band_roformer --start_check_point pretrain/{denoise_model_name}' + f' --input_folder {module_inputs["denoise_input"]} --store_dir denoise_results --extract_instrumental'
